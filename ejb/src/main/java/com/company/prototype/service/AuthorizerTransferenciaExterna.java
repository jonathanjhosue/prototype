...
@Named("authorizerTransferenciaExterna")
@SessionScoped
public class AuthorizerTransferenciaExterna implements Authorizer,Serializable{
	...
	final String transactionTypeCode= TiposTransaccion.TRANSFERENCIA_EXTERNA.getValue();
	...
	public AuthorizerTransferenciaExterna(){
	...
	public AuthorizationResult validation(Transaccion t) {
		...
		if(!transactionTypeCode.equals(t.getTipotransaccion())){
			...
			r.setResponseCode(AuthorizerResponse.INVALID_DATA);
			r.setMessage("Validations.Transaction.InternalAccount");			
		}
