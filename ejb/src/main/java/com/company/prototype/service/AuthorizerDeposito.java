...
@Named("authorizerDeposito")
@SessionScoped
public class AuthorizerDeposito implements Authorizer,Serializable{
	...
	final String transactionTypeCode= TiposTransaccion.DEPOSITO.getValue();
	...
	public AuthorizationResult process(Transaccion t) {
		...
		if(success.equals(r.getAuthorizerResponse())){			
			try{					
				...
				if(cuentaTo!=null){
					ec=estadoCuentaFacade.findCurrent(cuentaTo);	
				}				
				r=validationData(cuentaTo, ec,t);
